...
class AverageNumsStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, alb: elb.ApplicationLoadBalancer, cluster: ecs.Cluster, vpc: ec2.Vpc, listener: elb.ApplicationListener, svc_namespace: svc_discovery.INamespace, route_priority: int, **kwargs):
        ...
        EcsMicroService(self, "average-svc",
            name="average",
            uri_base="/average",
            directory=str(Path(__file__).resolve().parents[2].joinpath("average_nums")),
            alb=alb,
            cluster=cluster,
            ...
            service_dns_param_name=c.AWS_SSM_AVERAGE_ENDPOINT,
            container_env={
                "AWS_SSM_ADD_ENDPOINT": c.AWS_SSM_ADD_ENDPOINT,
                "AWS_SSM_DIVIDE_ENDPOINT": c.AWS_SSM_DIVIDE_ENDPOINT
            }
        )

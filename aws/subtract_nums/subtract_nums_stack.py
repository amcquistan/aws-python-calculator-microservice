...
class SubtractNumsStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, alb: elb.ApplicationLoadBalancer, cluster: ecs.Cluster, vpc: ec2.Vpc, listener: elb.ApplicationListener, svc_namespace: svc_discovery.INamespace, route_priority: int, **kwargs):
        ...
        EcsMicroService(self, "subtract-svc",
            name="subtract",
            uri_base="/subtract",
            directory=str(Path(__file__).resolve().parents[2].joinpath("subtract_nums")),
            alb=alb,
            cluster=cluster,
            ...
            service_dns_param_name=c.AWS_SSM_SUBTRACT_ENDPOINT
        )
